    ...
    public void sendScheduledNotifications() {
        ...
        List<Notification> scheduledNotifications =
                notificationRepository.findBySendTypeAndScheduledTimeLessThanEqualAndStatus(
                        ...
                        .stream()
                        .filter(n -> !n.getScheduledTime().isAfter(now))
                        .toList();
        ...
        notificationsToSend.addAll(scheduledNotifications);
        notificationsToSend.addAll(failedNotifications);

        notificationsToSend.forEach(
                notification -> eventPublisher.publishEvent(new NotificationSendEvent(notification)));
    }
